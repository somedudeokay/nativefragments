...
// Lighthouse (desktop) scores — verified 100 on every deployed demo — plus the
// runtime dependency count (excludes dev dependencies and the Native Fragments
// core, which every demo shares).
const metrics = [
  { value: "100", label: "Performance", kind: "lh" },
  { value: "100", label: "Accessibility", kind: "lh" },
  { value: "100", label: "Best practices", kind: "lh" },
  { value: "100", label: "SEO", kind: "lh" },
  { value: "0", label: "Dependencies", kind: "deps" },
];

const metricsHtml = () =>
  html`<ul class="demo-metrics" aria-label="Lighthouse scores and dependency count">
    ${raw(
      metrics
        .map(
          (m) => html`<li class="demo-metric demo-metric--${m.kind}">
            <span class="demo-metric__value">${m.value}</span>
            <span class="demo-metric__label">${m.label}</span>
          </li>`,
        )
        .join(""),
    )}
